...
<div class="sui-slider"
    ...
    <div class="sui-slider__track"
        ...
        <div class="sui-slider__slide"
            ...
            <?php if (!empty($row['linkURL'])): ?></a><?php endif; ?>
    ...
    <div class="sui-slider__controls">
        <?php if ($showArrows): ?>
        <button class="sui-slider__btn" @click="prev()" aria-label="<?= t('Previous slide') ?>">&#8592;</button>
        <?php endif; ?>
        ...
        <div class="sui-slider__dots">
            ...
            <button class="sui-slider__dot"
                    :class="{ 'is-active': current === <?= $i ?> }"
                    @click="goto(<?= $i ?>)"
                    aria-label="<?= h(t('Go to slide %s', $i + 1)) ?>"></button>
            <?php endfor; ?>
        </div>
        <?php endif; ?>

        <?php if ($showArrows): ?>
        <button class="sui-slider__btn" @click="next()" aria-label="<?= t('Next slide') ?>">&#8594;</button>
        <?php endif; ?>
    </div>
    <?php endif; ?>
